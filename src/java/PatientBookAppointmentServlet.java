// Replaced by patient.PatientBookAppointmentServlet in src/java/patient/
public class PatientBookAppointmentServlet {/*
    ...
    private void showCalendar(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // Get patient name from session for sidebar
            ...
            out.println("<a href='patient/dashboard' class='menu-item'>🔍 View Doctors</a>");
            out.println("<a href='patient/medical-details' class='menu-item'>🩺 Medical Info</a>");
            out.println("<a href='patient/appointments' class='menu-item'>📅 My Appointments</a>");
            out.println("<a href='patient/logout' class='logout-btn'>🚪 Logout</a>");
            out.println("</div>");
    ...
    private void generateCalendar(java.io.PrintWriter out, LocalDate month, int doctorId, Set<LocalDate> unavailableDates) {
        ...
        for (int day = 1; day <= daysInMonth; day++) {
            ...
            if (isPast || isUnavailable) {
                ...
                out.println("<button class='" + cssClass + "' onclick=\"location.href='patient/book-appointment?doctorId=" + doctorId + "&date=" + currentDate + "'\">" + day + "</button>");
            }
        }
    ...
    private void showAvailableSlots(HttpServletRequest request, HttpServletResponse response, String dateStr)
        ...
        try {
            ...
            out.println("</table>");
            out.println("<div class='back-link'><a href='patient/book-appointment?doctorId=" + doctorId + "'>← Change Date</a></div>");
            out.println("</div></div>");
            out.println("<div class='sidebar-overlay' onclick='toggleSidebar()'></div>");
            out.println("<script>");
            out.println("var isMobile = function(){ return window.innerWidth <= 768; };");
    ...
    private void handleBooking(HttpServletRequest request, HttpServletResponse response)
        ...
        // Get patient from session only - never trust hidden form field
        HttpSession session = request.getSession(false);
        if (session == null || session.getAttribute("username") == null) {
            response.sendRedirect("patient/login.html");
            return;
        ...
        try {
            ...
            // Check slot availability within the transaction
            ...
            if (existingCount > 0) {
                conn.rollback();
                response.sendRedirect("patient/book-appointment?doctorId=" + doctorId + "&date=" + dateStr + "&msg=slot_taken");
                return;
            }
            ...
            conn.commit();
            response.sendRedirect("patient/book-appointment?doctorId=" + doctorId + "&date=" + dateStr);
            ...
            response.getWriter().println("<a href='patient/book-appointment?doctorId=" + doctorId + "' style='color:#1D4ED8;text-decoration:none;background:#EFF6FF;padding:10px 20px;border-radius:8px;display:inline-block;border:1px solid #BFDBFE;font-weight:600'>Go Back</a>");
    ...
    private void handleCancellation(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            response.getWriter().println("<p>" + ex.getMessage() + "</p>");
            response.getWriter().println("<a href='patient/book-appointment?doctorId=" + doctorId + "&date=" + dateStr + "' style='color:#1D4ED8;text-decoration:none;background:#EFF6FF;padding:10px 20px;border-radius:8px;display:inline-block;border:1px solid #BFDBFE;font-weight:600'>Go Back</a>");
            response.getWriter().println("</body></html>");
        } finally {
            try {
                if (pstmt != null) pstmt.close();
