...
public class PatientAppointmentServlet extends HttpServlet {
    ...
    private void showAvailableSlots(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            out.println("</table>");
            out.println("<a href='dashboard' class='back-link'>Back to Dashboard</a>");
            out.println("</div>");
            ...
            response.getWriter().println("<a href='book-appointment?doctorId=" + doctorIdParam + "' style='color:white;display:block;text-align:center'>Try Again</a>");
        } finally {
            try {
                if (rs != null) rs.close();
    ...
    private void handleBooking(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (existingCount > 0) {
                conn.rollback();
                response.sendRedirect("book-appointment?doctorId=" + doctorId + "&msg=slot_taken");
                return;
            }
            ...
            response.getWriter().println("<br><a href='book-appointment?doctorId=" + doctorId + "' style='color:white;text-decoration:none;background:rgba(255,255,255,0.2);padding:12px 24px;border-radius:8px;display:inline-block'>Go Back</a>");
